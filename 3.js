...
const traverse = (inp, path) => {
    let direction = inp.substring(0, 1);
    let amount = parseInt(inp.substring(1));
    if (direction == "R") {
        for (let i=1; i<=amount; i++) {
            steps++;
            curX++;
            path[curX+","+curY] = steps;
        }
    } else if (direction == "L") {
        for (let i=1; i<=amount; i++) {
            steps++;
...
input1.forEach(x => traverse(x, path1));
curX = 0;
curY = 0;
steps = 0;
input2.forEach(x => traverse(x, path2));

let intersections = [];
Object.keys(path1).forEach(point => {
    if (path2[point]) {
